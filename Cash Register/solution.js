// price = purchase, cash = payment, cid = cash-in-drawer

function checkCashRegister(price, cash, cid) {
  let change = cash*100 - price*100; // js doesnt play nice with decimals thus convert to pennies 
  let cidTotal = 0;
  for(let elem of cid){
    cidTotal += elem[1]*100;
  }
  ...
  // if cash-in-drawer is equal to the change due 
    ...
    let answer = [];
     cid = cid.reverse(); // from highest to lowest
     let moneyUnits = {
       "ONE HUNDRED": 10000,
       "TWENTY": 2000,
       "TEN": 1000,
     ...
     for(let elem of cid){
       let holder = [elem[0], 0];
       elem[1] = elem[1]*100; // convert to pennies 
